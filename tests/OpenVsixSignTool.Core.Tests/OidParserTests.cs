namespace OpenVsixSignTool.Core.Tests
{
    ...
    public class OidParserTests
    {
        ...
        {
            System.Security.Cryptography.Oid oid = OidParser.ReadFromBytes(new byte[] { 6, 8, 42, 134, 72, 206, 61, 3, 1, 7 });
        ...
        {
            System.Security.Cryptography.Oid oid = OidParser.ReadFromBytes(new byte[] { 6, 9, 42, 134, 72, 206, 61, 3, 1, 7 });
            Assert.Null(oid);
        }
        ...
        {
            System.Security.Cryptography.Oid oid = OidParser.ReadFromBytes(new byte[] { 6, 8, 42, 134, 72, 206, 61, 3, 1, 255 });
            Assert.Null(oid);
        }
    }
}
