namespace OpenVsixSignTool.Core.Tests
{
    ...
    public class Crypt32Tests
    {
        ...
        public void ShouldTimestampData()
        {
            ...
            var ok = Crypt32.CryptRetrieveTimeStamp("http://timestamp.digicert.com", CryptRetrieveTimeStampRetrievalFlags.NONE, 30 * 1000, "1.3.14.3.2.26", ref parameters, data, (uint)data.Length, out CryptMemorySafeHandle pointer, IntPtr.Zero, IntPtr.Zero);
            Assert.True(ok);
            bool success = false;
            try
            {
                ...
                Assert.True(success);
                CRYPT_TIMESTAMP_CONTEXT structure = Marshal.PtrToStructure<CRYPT_TIMESTAMP_CONTEXT>(pointer.DangerousGetHandle());
